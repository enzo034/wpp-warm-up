


const randomWordsFromJs = {
    phrases: [
        "Hola, ¿cómo estás?",
        "¡Buenos días!",
        "¿Qué tal tu día?",
        "Espero que tengas una excelente semana.",
        "Nos vemos pronto.",
        "¡Qué interesante!",
        "Estoy aquí si necesitas algo.",
        "¿A qué hora nos conectamos?",
        "No olvides revisar los mensajes.",
        "Gracias por tu ayuda.",
        "¡Qué sorpresa!",
        "Cuéntame más sobre eso.",
        "¿Qué opinas de esto?",
        "Me parece una buena idea.",
        "¡Eso es genial!",
        "¿Te gustaría hacer algo hoy?",
        "Estoy de acuerdo contigo.",
        "¡Nos mantenemos en contacto!",
        "¿Ya viste las noticias?",
        "¡Disfruta tu día!",
        "Es un día perfecto para empezar algo nuevo.",
        "Confío en que todo saldrá bien.",
        "Recuerda tomar un descanso.",
        "Hablemos pronto.",
        "¡Hola! ¿Cómo te va?",
        "¿Todo bien por allá?",
        "Oye, ¿has visto esta película?",
        "Me alegra saber que estás bien.",
        "¡Te mando un abrazo!",
        "No olvides sonreír.",
        "Me encanta hablar contigo.",
        "¡Qué gusto verte!",
        "¡Qué lindo lugar!",
        "Nos vemos en un rato.",
        "¡Qué tal si nos reunimos más tarde?",
        "Hace mucho que no hablamos.",
        "¿Qué planes tienes para el fin de semana?",
        "Siempre es un placer charlar contigo.",
        "¡Me hiciste reír!",
        "Tengo una idea para mañana.",
        "Hoy ha sido un buen día.",
        "¿Me puedes dar tu opinión sobre esto?",
        "¡Qué emoción, ya casi llegamos!",
        "¿Qué tal si nos encontramos más tarde?",
        "No puedo esperar para ver lo que pasa después.",
        "A veces es bueno desconectar un poco.",
        "Espero que todo vaya bien en tu trabajo.",
        "¡Cuídate mucho!",
        "Es un buen momento para relajarse.",
        "Ojalá podamos vernos pronto.",
        "Estoy listo para escuchar tus ideas.",
        "¿Qué te gustaría hacer hoy?",
        "No puedo esperar para saber más.",
        "Me encanta escuchar sobre tus experiencias.",
        "Es un placer ayudarte.",
        "Gracias por todo lo que haces.",
        "¡Qué día tan bonito!",
        "¡Qué bueno verte!",
        "No te olvides de disfrutar el momento.",
        "¡Qué sorpresa verte por aquí!",
        "Siempre es bueno tener un buen amigo.",
        "Nunca dejes de soñar.",
        "¿Te gustaría salir este fin de semana?",
        "Hoy está siendo un día perfecto.",
        "Me alegra mucho verte feliz.",
        "Es un buen día para empezar algo nuevo.",
        "¡Qué buena noticia!",
        "¿Estás listo para mañana?",
        "¿Qué tal el fin de semana?",
        "Estoy emocionado por lo que viene.",
        "Espero que tu día sea excelente.",
        "Gracias por tu apoyo.",
        "Ojalá todo salga bien para ti.",
        "¡Qué divertido es hablar contigo!",
        "Espero que tengas un buen descanso.",
        "Me encanta compartir ideas contigo.",
        "¡Eso me hizo pensar en algo interesante!",
        "Vamos a aprovechar este día.",
        "Es genial estar en contacto.",
        "Gracias por hacerme reír.",
        "Me encantan tus historias.",
        "Siempre aprendo algo nuevo de ti.",
        "Hoy es un día perfecto para comenzar proyectos.",
        "No olvides cuidar de ti mismo.",
        "Estoy aquí si me necesitas.",
        "¡Qué buen clima hoy!",
        "Siempre es agradable hablar contigo.",
        "Es un buen día para salir a caminar.",
        "Me siento muy afortunado de tenerte como amigo.",
        "Estoy disfrutando mucho de esta conversación.",
        "Estoy muy agradecido por todo.",
        "Qué bueno verte de nuevo.",
        "¡Qué emoción saber de ti!",
        "Es un placer ayudarte en lo que necesites.",
        "Nunca dejes de sonreír.",
        "Es impresionante lo que puedes lograr.",
        "Gracias por ser tan increíble.",
        "¡Qué bien que pudimos hablar!",
        "No puedo esperar para ponernos al día.",
        "¡Qué maravilloso es este momento!",
        "A veces, un buen descanso es lo que necesitamos.",
        "Gracias por compartir tus pensamientos.",
        "Espero verte pronto.",
        "Siempre es un placer conversar contigo.",
        "Hoy ha sido un día lleno de sorpresas.",
        "Gracias por escucharme.",
        "¡Qué increíble día está siendo!",
        "Espero que todo te salga bien.",
        "¡Qué día tan especial!",
        "Hoy es un día perfecto para empezar algo nuevo.",
        "Te deseo lo mejor en todo.",
        "¡Qué suerte tener un día tan bonito!",
        "Espero que todo vaya de maravilla.",
        "¡Qué gran noticia!",
        "Es el momento ideal para comenzar.",
        "Siempre es un placer ayudarte.",
        "¡Qué bonito clima hoy!",
        "Te mando mucho ánimo.",
        "Qué bueno que todo esté bien.",
        "¡Qué ganas de verte!",
        "Te espero con mucho gusto.",
        "Gracias por ser tan paciente.",
        "Nunca dejes de buscar nuevas oportunidades.",
        "Ojalá podamos pasar más tiempo juntos.",
        "Qué bueno tener una charla tan amena.",
        "Me encantaría que sigamos en contacto.",
        "Me ha gustado mucho lo que me dijiste.",
        "Vamos a lograr cosas grandes juntos.",
        "Estoy emocionado por lo que vendrá.",
        "Espero que este día sea tan bueno como el anterior.",
        "Es genial pasar el tiempo contigo.",
        "Sigue así, vas muy bien.",
        "No te olvides de disfrutar la vida.",
        "Todo lo que haces tiene mucho valor.",
        "Es impresionante lo que puedes lograr.",
        "Espero que todo se resuelva de la mejor manera."
    ]
}

module.exports = { randomWordsFromJs };